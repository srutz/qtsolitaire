...
SolitaireWidget::SolitaireWidget(Game *game, QWidget *parent) : QWidget(parent), m_game(game)
{
    ...
    // Set scene rect to start at (0,0) and cover a reasonable game area
    ...
    m_graphicsView->setAlignment(Qt::AlignTop | Qt::AlignLeft);
    m_graphicsView->setResizeAnchor(QGraphicsView::NoAnchor);
...
void SolitaireWidget::layoutGame(bool delayed)
{
    ...
    auto layoutPile = [this, delayed, &globalZValue](const Pile &pile, QPoint pos) {
        ...
        for (const auto &card : pile.cards) {
            ...
            if (delayed) {
                // Calculate an offscreen position based on widget dimensions
