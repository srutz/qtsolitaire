...
template <typename T>
void runInBackground(QObject *mainThreadContext, std::function<T()> workFunc, std::function<void(T)> resultCallback)
{
    ...
    QObject::connect(thread, &QThread::started, context, [context, workFunc, resultCallback, thread, safeContext]() {
        T result = workFunc();
